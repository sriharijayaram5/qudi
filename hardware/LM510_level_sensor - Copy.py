...
class LM510levelsensor(Base, SimpleDataInterface):
    ...
    def on_activate(self):
        ...
        try:
            ...
            self.log.error('Connection to the LM510 level sensor failed. Could not connect to the address >>{}<<.'.format(self.serial_port))
            raise
        
        model = self._connection.query('*IDN?').split(',')[1]
