...
class Levelmeter(Base, SimpleDataInterface):
    """ Read human readable numbers from serial port.

    Example config for copy-paste:

    level_sensor:
        module.Class: 'helium_meter_LM500.LM500_meter.Levelmeter'
        serial_port: 'COM4'

    """
    serial_port = ConfigOption('serial_port', 'COM4', missing='warn')

    def on_activate(self):
        """ Activate module.
        """
        try:
            self.level_sensor = Cryomagnetics.LM500(self.serial_port)
        except:
            self.log.error('Connection to the LM500 level sensor failed.')
