...
class APSMagnet(Base, MagnetInterface):
    ...
    aps100:
        ...
        magnet_address_yx: 'COM8'
        magnet_address_z: 'COM9'
    ...
    """
    # config opts
    addr_yx = ConfigOption('magnet_address_yx', missing='error')
    ...
    def on_activate(self):
        ...
        """
        self.ser_yx = serial.Serial(port=self.addr_yx, baudrate=9600, bytesize=8, timeout=2, stopbits=serial.STOPBITS_ONE)
        self.ser_z = serial.Serial(port=self.addr_z, baudrate=9600, bytesize=8, timeout=2, stopbits=serial.STOPBITS_ONE)
...
# =========================== Magnet Functionality Core ====================================
    ...
    def tell(self, param_dict):
        ...
        if param_dict.get('x') is not None:
            if not param_dict['x'].endswith('\n'):
                param_dict['x'] += '\n'
            self.ser_yx.write(self.utf8_to_byte('CHAN 2\n'))
            self.ser_yx.readline().decode()
            self.ser_yx.write(self.utf8_to_byte(param_dict['x']))
            self.ser_yx.readline().decode()
            internal_counter += 1
        if param_dict.get('y') is not None:
            if not param_dict['y'].endswith('\n'):
                param_dict['y'] += '\n'
            self.ser_yx.write(self.utf8_to_byte('CHAN 1\n'))
            self.ser_yx.readline().decode()
            self.ser_yx.write(self.utf8_to_byte(param_dict['y']))
            self.ser_yx.readline().decode()
            internal_counter += 1
        if param_dict.get('z') is not None:
            if not param_dict['z'].endswith('\n'):
                param_dict['z'] += '\n'
            self.ser_z.write(self.utf8_to_byte(param_dict['z']))
            self.ser_z.readline().decode()
            internal_counter += 1
    ...
    def ask(self, param_dict):
        ...
        if param_dict.get('x') is not None:
            ...
            answer_dict['x'] = self.byte_to_utf8(self.ser_yx.readline())  # receive an answer
        ...
        if param_dict.get('y') is not None:
            ...
            answer_dict['y'] = self.byte_to_utf8(self.ser_yx.readline())  # receive an answer
        ...
        if param_dict.get('z') is not None:
            ...
            answer_dict['z'] = self.byte_to_utf8(self.ser_z.readline())  # receive an answer
